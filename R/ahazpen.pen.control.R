...
"lasso.control"<-function(alpha = 1, ada.wgt=NULL)
  {
    ...
    # Prefactor for covariate weighting - always 1 for sscad
    ada.wgt<-function(surv,X,weights){return(rep(1,ncol(cbind(X))))}
    init.sol<-function(surv,X,weights){return(rep(0,ncol(cbind(X))))}
    ...
    out<-list("type"="lasso","init.sol"=init.sol,"alpha"=alpha,"nsteps" = 1,"a"=1,"c"=NULL,"ada.wgt"=ada.wgt)
    class(out)<-"ahaz.pen.control"
    return(out)
  }
...
"sscad.control"<-function(a = 3.7,nsteps = 1,init.sol=NULL, c=NULL)
  {
    ...
    out<-list(type="sscad","init.sol"=init.sol,"alpha"=1,"nsteps" = nsteps,"a"=a,"c"=c,"ada.wgt"=ada.wgt)
    class(out)<-"ahaz.pen.control"
    return(out)
  }
